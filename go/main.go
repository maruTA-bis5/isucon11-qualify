...
func getUserIDFromSession(c echo.Context) (string, int, error) {
	...
	err = db.GetContext(c.Request().Context(), &count, "SELECT COUNT(*) FROM `user` WHERE `jia_user_id` = ?",
		jiaUserID)
	if err != nil {
...
func postAuthentication(c echo.Context) error {
	...
	_, err = db.ExecContext(c.Request().Context(), "INSERT IGNORE INTO user (`jia_user_id`) VALUES (?)", jiaUserID)
	if err != nil {
		c.Logger().Errorf("db error: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func postIsu(c echo.Context) error {
	...
	_, err = tx.ExecContext(c.Request().Context(), "UPDATE `isu` SET `character` = ? WHERE  `jia_isu_uuid` = ?", isuFromJIA.Character, jiaIsuUUID)
	if err != nil {
		c.Logger().Errorf("db error: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func getIsuID(c echo.Context) error {
	...
	var res Isu
	err = db.GetContext(c.Request().Context(), &res, "SELECT * FROM `isu` WHERE `jia_user_id` = ? AND `jia_isu_uuid` = ?",
		jiaUserID, jiaIsuUUID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func getIsuGraph(c echo.Context) error {
	...
	var count int
	err = tx.GetContext(c.Request().Context(), &count, "SELECT COUNT(*) FROM `isu` WHERE `jia_user_id` = ? AND `jia_isu_uuid` = ?",
		jiaUserID, jiaIsuUUID)
	if err != nil {
		c.Logger().Errorf("db error: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	...
	res, err := generateIsuGraphResponse(c.Request().Context(), tx, jiaIsuUUID, date)
	if err != nil {
		c.Logger().Error(err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func generateIsuGraphResponse(ctx context.Context, tx *sqlx.Tx, jiaIsuUUID string, graphDate time.Time) ([]GraphResponse, error) {
	...
	rows, err := tx.QueryxContext(ctx, "SELECT * FROM `isu_condition` WHERE `jia_isu_uuid` = ? ORDER BY `timestamp` ASC", jiaIsuUUID)
	if err != nil {
...
func getIsuConditions(c echo.Context) error {
	...
	var isuName string
	err = db.GetContext(c.Request().Context(), &isuName,
	...
	conditionsResponse, err := getIsuConditionsFromDB(c.Request().Context(), db, jiaIsuUUID, endTime, conditionLevel, startTime, conditionLimit, isuName)
	if err != nil {
		c.Logger().Errorf("db error: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func bulkLoadIsu(c echo.Context) (map[string][]Isu, error) {
	chairs := []Isu{}
	err := db.SelectContext(c.Request().Context(), &chairs, "SELECT * FROM isu")
	if err != nil {
		return nil, err
	}
	chairsByCharacter := make(map[string][]Isu)
	for _, chair := range chairs {
		character := chair.Character
		forCharacter := chairsByCharacter[character]
		if forCharacter == nil {
			forCharacter = []Isu{}
		}
		chairsByCharacter[character] = append(forCharacter, chair)
	}
	return chairsByCharacter, nil
}

func bulkLoadConditions(c echo.Context, chairUUIDs []string) (map[string]IsuCondition, error) {
	if len(chairUUIDs) == 0 {
	...
	conditions := []IsuCondition{}
	err = db.SelectContext(c.Request().Context(), &conditions, query, params...)
	if err != nil {
		return nil, err
	}
	condByIsu := make(map[string]IsuCondition)
	var currentIsu string
	for _, cond := range conditions {
		if currentIsu == "" {
			currentIsu = cond.JIAIsuUUID
		}
		if currentIsu == cond.JIAIsuUUID {
			continue
		}
		condByIsu[currentIsu] = cond
	}
	return condByIsu, nil
}

// GET /api/trend
// ISUの性格毎の最新のコンディション情報
func getTrend(c echo.Context) error {
	chairsByCharacter, err := bulkLoadIsu(c)
	if err != nil {
		c.Logger().Errorf("db error: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
	...
	for character, chairs := range chairsByCharacter {
		isuList := chairs
		...
		condByIsu, err := bulkLoadConditions(c, isuIDs)
		if err != nil {
			c.Logger().Errorf("db error: %v", err)
			return c.NoContent(http.StatusInternalServerError)
		}
		for _, isu := range isuList {
			isuLastCondition := condByIsu[isu.JIAIsuUUID]
...
func postIsuCondition(c echo.Context) error {
	...
	var count int
	err = tx.GetContext(c.Request().Context(), &count, "SELECT COUNT(*) FROM `isu` WHERE `jia_isu_uuid` = ?", jiaIsuUUID)
	if err != nil {
		c.Logger().Errorf("db error: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
